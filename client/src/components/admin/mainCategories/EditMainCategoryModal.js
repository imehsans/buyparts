...
import { editMainCategory, getAllMainCategory } from "./FetchApi";
import { X, Save } from "lucide-react";

const EditMainCategoryModal = (props) => {
  ...
  useEffect(() => {
    setDes(data.editMainCategoryModal.des);
    setStatus(data.editMainCategoryModal.status);
    setMId(data.editMainCategoryModal.mId);

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data.editMainCategoryModal.modal]);
  ...
  const submitForm = async () => {
    ...
    let edit = await editMainCategory(mId, des, status);
    if (edit.error) {
      console.log(edit.error);
      dispatch({ type: "loading", payload: false });
    } else if (edit.success) {
      console.log(edit.success);
      dispatch({ type: "editMainCategoryModalClose" });
      setTimeout(() => {
        fetchData();
        dispatch({ type: "loading", payload: false });
      }, 1000);
  ...
  return (
    <Fragment>
      {/* Black Overlay */}
      <div
        onClick={(e) => dispatch({ type: "editMainCategoryModalClose" })}
        className={`${data.editMainCategoryModal.modal ? "" : "hidden"
      ...
      >
        <div className="relative bg-dark-card w-full md:w-3/6 shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 rounded-2xl flex flex-col items-center space-y-4 overflow-y-auto px-4 py-8 md:px-8 animate-in zoom-in-95 duration-200">
          ...
          <div className="flex flex-col space-y-2 w-full mt-4">
            <label htmlFor="description" className="text-sm font-medium text-gray-400 uppercase tracking-widest">Main Category Description</label>
            <textarea
              value={des}
